...
## dat <- fread('/scratch2/bldinh/assoc_testing/data/mega_and_gda_king-initial_king_ibdseg.seg.gz')
...
if (kin.type == "king_ibdseg") {
    kinship <- read_tsv(config["kinship_file"], col_types="-c-cnnn-") %>% mutate(IBS0=(1 - IBD1Seg - IBD2Seg), kinship=0.5*PropIBD)
    xvar <- "IBS0"
...
p <- ggplot(kinship, aes(.data[[xvar]], .data[["kinship"]])) +
    geom_hline(yintercept=2^(-seq(3,11,2)/2), linetype="dashed", color="grey") +
    ## geom_point(alpha=0.5) +
    ## theme_bw()
    geom_hex(aes(fill = log10(after_stat(count))), bins = 100) +
...
if (!is.na(config["phenotype_file"]) & !is.na(config["study"])) {
    ...
    p <- ggplot(kinship.study, aes(.data[[xvar]], .data[["kinship"]])) +
        geom_hline(yintercept=2^(-seq(3,11,2)/2), linetype='dashed', color="grey") +
        ## geom_point(alpha=0.5) +
        ## theme_bw()
        facet_wrap(~study) +
    ...
    if (kin.type %in% c("king_ibdseg", "pcrelate")) {
        ...
        if (nrow(kinship.cross) > 0){
            p <- ggplot(kinship.cross, aes(.data[[xvar]], .data[[kinship]], .data[[color="study2"]])) +
                geom_hline(yintercept=2^(-seq(3,11,2)/2), linetype='dashed', color="grey") +
                geom_point() +
